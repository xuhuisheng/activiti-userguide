...
与Spring集成后，业务流程可以非常简单地使用标准的 <<apiUnitTesting,Activiti测试工具Activiti testing facilities>>进行测试。下面的例子展示了如何通过典型的基于Spring的单元测试，对业务流程进行测试：
...
上面也提到过，Spring Boot的约定大雨配置。默认情况下，如果classpath中只有H2，就会创建内存数据库，并传递给Activiti流程引擎配置。

要更换数据源，简单地通过提供Datasource bean覆盖默认配置。我们在这里使用__DataSourceBuilder__类，这是Spring Boot的辅助类。如果classpath中有Tomcat, HikariCP 或者 Commons DBCP，会（按照这个顺序，先是Tomcat）选择一个。例如，要切换到MySQL数据库：
...
通常在嵌入的Activiti引擎之上，需要REST API（与公司的不同服务交互）。Spring Boot让这个变得很容易。在classpath中添加下列依赖：
...
很明显还有很多Spring　Boot相关的内容还没有提及，例如简单的JTA集成，构建能在主流应用服务器上运行的war文件。还有很多关于Spring Boot的内容：
